...
const { METHOD_RETURN, ERROR, SIGNAL, METHOD_CALL } = dbus.MessageType
const {
  isObjectPathValid,
  isMemberNameValid,
...
program
  ...
  .option(
    "-o, --output [path]",
    "The output file or directory (default: stdout)",
  )
  .option(
    "-r, --recursive",
    "Recursively decent to child nodes (requires --output to be directory",
  )
...
if (path.basename(program.template, ".hbs") === program.template) {
  program.template = path.resolve(
    __dirname,
    "..",
    "templates",
    path.basename(program.template, ".hbs") + ".hbs",
  )
}
...
async function main() {
  ...
  if (program.recursive) {
    ...
    while (nodeList.length > 0) {
      ...
      nodeList.push(
        ...interfaceData.subNodes.map((name) => currentNode + "/" + name),
      )
      for (const ifs of interfaceData.interfaces) {
        const foundInterface = interfaceList.find(
          (i) => i.name === ifs["$"].name,
        )
        if (foundInterface) {
