...
const { _getJSBIConstants } = require("../../lib/constants")
...
class LongInterface extends Interface {
  EchoSigned(what) {
    if (what.prototype !== JSBI.BigInt.prototype) {
      throw new DBusError(
        TEST_ERROR_PATH,
        "interface with long compat expected a JSBI BigInt for type x",
      )
    }
    return what
  }
  ...
  EchoUnsigned(what) {
    if (what.prototype !== JSBI.BigInt.prototype) {
      throw new DBusError(
        TEST_ERROR_PATH,
        "interface with long compat expected a JSBI BigInt for type t",
      )
    }
    return what
  }
