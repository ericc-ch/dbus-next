...
function marshallVariantToJs(variant) {
  ...
  if (!type.child.length) {
    if (valueIsMarshallVariant(value)) {
      return new Variant(
        collapseSignature(value[0][0]),
        marshallVariantToJs(value),
      )
    } else {
      return value
    }
  ...
  if (type.type === "a") {
    if (type.child[0].type === "y") {
      ...
      for (let i = 0; i < value.length; ++i) {
        ...
        result[value[i][0]] = marshallVariantToJs([
...
function jsToMarshalFmt(signature, value) {
  ...
  if (
    ...
    if (value.length !== signature.child.length) {
      throw new Error(
        `expecting struct to have ${signature.child.length} members (got ${value.length} members)`,
      )
    }
    const result = []
    for (let i = 0; i < value.length; ++i) {
  ...
  for (let i = 0; i < body.length; ++i) {
    if (signatureTree[i].type === "v") {
      if (body[i].constructor !== Variant) {
        throw new Error(
          `Expected a Variant() argument for position ${i + 1} (value='${body[i]}')`,
        )
      }
