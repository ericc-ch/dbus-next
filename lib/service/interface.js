 ...
 * interfaces. Use the {@link module:interface~Interface.configureMembers}
 * method to define the properties, methods, and signals that the interface has.
 * These will be advertised to users in the introspection xml gotten by the
 * `org.freedesktop.DBus.Introspect` method on the name.
 *
 * @example
...
class Interface {
  ...
  /**
   * Configure the DBus members (properties, methods, signals) for this Interface.
   *
   * @static
   * @example
   ...
   * @param members.properties {Object} - The class members to define as DBus
   * properties. The key should be a property or getter/setter defined on the
   * class. The value should be an object with the following options:
   * - `signature` {string} - The DBus type signature for this property (required).
   * - `access` {string} - The read/write access: ACCESS_READ, ACCESS_WRITE, or ACCESS_READWRITE (default).
   * - `name` {string} - The name on the bus (defaults to the member name).
   * - `disabled` {boolean} - Whether this property is hidden from the bus (default: false).
   * @param members.methods {Object} - The class methods to define as DBus
   * methods. The key should be a method defined on the class. The value should
   * be an object with the following options:
   * - `inSignature` {string} - The DBus type signature for input parameters (default: '').
   * - `outSignature` {string} - The DBus type signature for the return value (default: '').
   * - `name` {string} - The name on the bus (defaults to the member name).
   * - `disabled` {boolean} - Whether this method is hidden from the bus (default: false).
   * - `noReply` {boolean} - Whether this method expects no reply (default: false).
   * @param members.signals {Object} - The class methods to define as DBus signals.
   * The key should be a method defined on the class. The value should be an
   * object with the following options:
   * - `signature` {string} - The DBus type signature for signal parameters (default: '').
   * - `name` {string} - The name on the bus (defaults to the member name).
   * - `disabled` {boolean} - Whether this signal is hidden from the bus (default: false).
   */
  static configureMembers (members) {
    ...
    for (const k of Object.keys(properties)) {
      ...
      this.prototype.$properties[k] = options;
    }
    ...
    for (const k of Object.keys(methods)) {
      ...
      this.prototype.$methods[k] = options;
    }
    ...
    for (const k of Object.keys(signals)) {
      ...
      this.prototype.$signals[k] = options;
    }
  }
