...
class ServiceObject {
  ...
  addInterface(iface) {
    if (!(iface instanceof Interface)) {
      throw new Error(
        `object.addInterface takes an Interface as the first argument (got ${iface})`,
      )
    }
    if (this.interfaces[iface.$name]) {
      throw new Error(
        `an interface with name '${iface.$name}' is already exported on this object`,
      )
    }
    this.interfaces[iface.$name] = iface
    const that = this

    const propertiesChangedHandler = function (
      changedProperties,
    ...
    const signalHandler = function (options, result) {
      ...
      that.bus.send(
        Message.newSignal(that.path, iface.$name, name, signature, result),
      )
    }
  ...
  removeInterface(iface) {
    if (!(iface instanceof Interface)) {
      throw new Error(
        `object.removeInterface takes an Interface as the first argument (got ${iface})`,
      )
    }
    if (!this.interfaces[iface.$name]) {
