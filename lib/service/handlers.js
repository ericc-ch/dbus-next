...
function sendServiceError(bus, msg, errorMessage) {
  bus.send(
    Message.newError(
      msg,
      "com.github.dbus_next.ServiceError",
...
function handleGetProperty(bus, msg, path) {
  ...
  if (!iface) {
    bus.send(
      Message.newError(msg, INVALID_ARGS, `No such interface: '${ifaceName}'`),
    )
    return
  }
  ...
  if (propertyValue instanceof DBusError) {
    ...
    return sendServiceError(
      bus,
      msg,
      "tried to get a property that is not set: " + prop,
    )
  }

  if (
    !(options.access === ACCESS_READWRITE || options.access === ACCESS_READ)
  ) {
    bus.send(
      Message.newError(
        msg,
    ...
    for (const k of Object.keys(properties)) {
      ...
      if (value instanceof DBusError) {
        ...
        return sendServiceError(
          bus,
          msg,
          "tried to get a property that is not set: " + p,
        )
      }
...
function handleSetProperty(bus, msg, path) {
  ...
  if (
    !(options.access === ACCESS_WRITE || options.access === ACCESS_READWRITE)
  ) {
    bus.send(
      Message.newError(
        msg,
...
function handleStdIfaces(bus, msg) {
  ...
  if (!isInterfaceNameValid(ifaceName)) {
    bus.send(
      Message.newError(
        msg,
        INVALID_ARGS,
        `Invalid interface name: '${ifaceName}'`,
      ),
    )
    return true
  }
  ...
  if (!isMemberNameValid(member)) {
    bus.send(
      Message.newError(msg, INVALID_ARGS, `Invalid member name: '${member}'`),
    )
    return true
  }
  ...
  if (!isObjectPathValid(path)) {
    bus.send(
      Message.newError(msg, INVALID_ARGS, `Invalid path name: '${path}'`),
    )
    return true
  }
