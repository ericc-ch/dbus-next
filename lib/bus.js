...
class MessageBus extends EventEmitter {
  ...
  call(msg) {
    return new Promise((resolve, reject) => {
      if (!(msg instanceof Message)) {
        throw new Error(
          "The call() method takes a Message class as the first argument.",
        )
      }
      if (msg.type !== METHOD_CALL) {
        throw new Error(
          "Only messages of type METHOD_CALL can expect a call reply.",
        )
      }
  ...
  send(msg) {
    if (!(msg instanceof Message)) {
      throw new Error(
        "The send() method takes a Message class as the first argument.",
      )
    }
    if (msg.serial === null || msg._sent) {
      msg.serial = this.newSerial()
