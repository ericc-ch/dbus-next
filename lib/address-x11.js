...
function getDbusAddressFromWindowSelection(callback) {
  ...
  if (x11 === null) {
    throw new Error(
      "cannot get session bus address from window selection: dbus-next was installed without x11 support",
    )
  }
...
function getDbusAddressFromFs() {
  ...
  if (!display) {
    throw new Error(
      "could not get DISPLAY environment variable to get dbus address",
    )
  }
  ...
  if (!match || !match[1]) {
    throw new Error(
      "could not parse DISPLAY environment variable to get dbus address",
    )
  }
  ...
  const machineId = fs
    ...
    .trim()
  const dbusInfo = fs
    .readFileSync(`${home}/.dbus/session-bus/${machineId}-${displayNum}`)
    .toString()
    .trim()
  for (let line of dbusInfo.split("\n")) {
    ...
    if (line.startsWith("DBUS_SESSION_BUS_ADDRESS=")) {
      ...
      if (!address) {
        throw new Error(
          "DBUS_SESSION_BUS_ADDRESS variable is set incorrectly in dbus info file",
        )
      }
