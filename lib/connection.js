      ...
      switch (family.toLowerCase()) {
        ...
        case "unix": {
          ...
          if (params.abstract) {
            ...
            const sock = new usocket.USocket({
              path: "\u0000" + params.abstract,
            })
...
function createConnection(opts) {
  ...
  stream.on("end", function () {
    ...
    self.message = function () {
      self.emit(
        "error",
        new Error("Tried to write a message to a closed stream"),
      )
    }
  })
      ...
      function (message) {
        try {
          ...
          self.emit(
            "error",
            err,
            `There was an error receiving a message (this is probably a bug in dbus-next): ${message}`,
          )
          return
        }
  ...
  self.once("connect", function () {
    ...
    self.message = function (msg) {
      ...
      if (stream.supportsUnixFd) {
        ...
        if (fds.length > 0) {
          console.warn(
            "Sending file descriptors is not supported in current bus connection",
          )
        }
        stream.write(data)
      }
